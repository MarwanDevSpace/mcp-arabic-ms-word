...
exports.BidiOpenXmlSurgeon = void 0;
const jszip_1 = __importDefault(require("jszip"));
const node_fs_1 = __importDefault(require("node:fs"));
const errors_js_1 = require("../core/errors.js");
const logger_js_1 = require("../core/logger.js");
class BidiOpenXmlSurgeon {
    /**
     * Performs deep OpenXML surgical repair on an Arabic DOCX file
     */
    async enforceBidiAndTypography(filePath, options = {}, outputPath) {
        if (!node_fs_1.default.existsSync(filePath)) {
            throw new errors_js_1.FileOperationError(`Target document not found: '${filePath}'`);
        }
        const { fixHeadingsAlignment = true, justifyBodyParagraphs = true, preventVerseSplitting = true, injectDynamicPageNumbering = true, isolateEnglishSections = true, } = options;
        try {
            ...
            docXml = docXml.replace(/<w:p\b[\s\S]*?<\/w:p>/g, (paragraphXml) => {
                ...
                const isHeading = paragraphXml.includes('w:val="Heading') ||
                    paragraphXml.includes('w:val="heading') ||
                    /^((المبحث|المطلب|الفصل|الباب|تمهيد|المقدمة|الخاتمة|الفهرس|المراجع|عنوان|أولاً|ثانياً|ثالثاً|رابعاً|خامساً)[\s:])/i.test(plainText);
                // Check if paragraph contains Quranic verse or Hadith
                const isVerseOrHadith = plainText.includes('﴿') ||
                    plainText.includes('﴾') ||
            ...
            node_fs_1.default.writeFileSync(finalOutputPath, newBuffer);
            logger_js_1.Logger.info(`Enforced Arabic BiDi and OpenXML typography on '${finalOutputPath}'`);
            return {
            ...
            throw new errors_js_1.XmlManipulationError(`Failed to enforce BiDi and typography on '${filePath}': ${msg}`);
        }
    }
    setJc(pPr, alignment) {
    ...
    async injectPageNumberingXml(zip) {
        try {
...
exports.BidiOpenXmlSurgeon = BidiOpenXmlSurgeon;
//# sourceMappingURL=bidi_openxml_surgeon.js.map