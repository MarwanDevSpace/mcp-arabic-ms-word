...
export async function resolveAndExecuteIntent(
  request: AutomatedIntentRequest
): Promise<AutomatedIntentResult> {
  ...
  const resolvedPath = resolveWorkspacePath(fileName);
  const font = request.fontFamily || (archetype === 'executive_report' || archetype === 'policy_manual' ? 'Cairo' : 'Amiri');

  Logger.info(`Dynamically resolving document intent '${archetype}' for prompt: '${request.prompt}'`);
