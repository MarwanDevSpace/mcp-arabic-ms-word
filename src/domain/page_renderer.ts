...
import { ArabicXmlEngine } from './xml_engine.js';
...
export class DocumentPageRenderer {
  /**
   ...
   */
  public async auditAndRenderPages(
    docxPath: string,
    options: PageAuditOptions = {}
  ): Promise<DocumentPageAuditResult> {
    if (!fs.existsSync(docxPath)) {
      throw new FileOperationError(`Document not found: '${docxPath}'`);
    }

    const {
      outputFolderName = 'Pages',
      dpi = 150,
      detectLayoutDefects = true,
    } = options;

    const docDir = path.dirname(docxPath);
    const baseName = path.basename(docxPath, path.extname(docxPath));
    const pagesDir = path.join(docDir, outputFolderName);
    const pdfPath = path.join(docDir, `${baseName}.pdf`);
    ...
    Logger.info(`Auditing and rendering pages for '${docxPath}' into '${pagesDir}'`);

    let pageCount = 1;
    const renderedPages: RenderedPageInfo[] = [];
    ...
    if (isWindows) {
      try {
        ...
        Logger.warn(`Native Windows rendering encountered an issue: ${msg}. Proceeding with diagnostic layout analysis.`);
      }
    }
    ...
    if (fs.existsSync(pagesDir)) {
      ...
      if (pageFiles.length > 0) {
        ...
        pageFiles.forEach((file, index) => {
          const imgPath = path.join(pagesDir, file);
          renderedPages.push({
            pageNumber: index + 1,
            imagePath: imgPath,
    ...
    if (renderedPages.length === 0) {
      ...
      for (let i = 1; i <= pageCount; i++) {
        ...
        const placeholderPath = path.join(pagesDir, placeholderName);
...
`;
    ...
    execSync(`powershell -NoProfile -EncodedCommand ${encoded}`, { timeout: 30000 });
    return true;
  }

  private async estimatePageCountFromXml(docxPath: string): Promise<number> {
    try {
  ...
  private async analyzeLayoutDefects(
    docxPath: string,
    detectDefects: boolean,
    pageCount: number
  ): Promise<DocumentPageAuditResult['diagnostics']> {
    if (!detectDefects) {
      return {
