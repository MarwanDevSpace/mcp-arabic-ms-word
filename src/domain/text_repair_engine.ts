...
export class ArabicTextRepairEngine {
  public static repairText(text: string, options: TextRepairOptions = {}): TextRepairResult {
    let current = text;
    const applied: string[] = [];

    const opts: Required<TextRepairOptions> = {
    ...
    if (opts.normalizeAlef) {
      ...
      if (before !== current) applied.push('Normalized Alef forms (أ, إ, آ -> ا)');
    }
    ...
    if (opts.normalizeYeh) {
      ...
      if (before !== current) applied.push('Normalized Dotless Yeh (ى -> ي)');
    }
    ...
    if (opts.standardizeDigits === 'eastern') {
      ...
      if (before !== current) applied.push('Converted Western digits to Eastern Arabic (٠-٩)');
      ...
      if (before !== current) applied.push('Converted Eastern digits to Western Arabic (0-9)');
    }
    ...
    if (opts.fixInvertedPunctuation) {
      ...
      current = current
        .replace(/([ا-ي])\s*\(/g, '$1 (')
        .replace(/\)\s*([ا-ي])/g, ') $1');
      if (before !== current) applied.push('Fixed RTL punctuation & parenthesis spacing');
    }
    ...
    if (opts.trimExtraSpaces) {
      ...
      if (before !== current) applied.push('Trimmed redundant whitespace and tabs');
    }
